...
public class ShareMenuInviteGolferDetail implements OnClickListener {
	...
	String 									content = "从爱高尔夫分享,大厅约球";

	public ShareMenuInviteGolferDetail(Activity context, View view, String inviter_sn,String app_sn) {
		...
		String param= null;
		try {
			param = "appSn="+app_sn+"&sn="+ MainApp.getInstance().getCustomer().sn+"&appVersion="+mContext.getPackageManager().getPackageInfo(context.getPackageName(),0).versionCode+"&apiVersion="+200;
		} catch (PackageManager.NameNotFoundException e) {
			e.printStackTrace();
		}

		DebugTools.getDebug().debug_v("getShareURL", "----->>>>"+param);
		imageUrl = Utils.getAvatarURLString(inviter_sn);
		
		try {
			targetUrl = BaseRequest.SERVER_PATH+"share/shareApplicationDetail?param="+URLEncoder.encode(Base64.encode(param.getBytes()),HTTP.UTF_8);
