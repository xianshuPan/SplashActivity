...
public class PublishNotifier extends Notifier {
	...
	public void notifyTimingPublish(PublishBean bean) {
		...
		// 延迟一秒后取消通知
		MainApp.getInstance().getHandler().removeCallbacks(removeTimingPublishRunnable);
		MainApp.getInstance().getHandler().postDelayed(removeTimingPublishRunnable, 1000);
	}
	...
	public void notifyPrePublish(PublishBean bean) {
		...
		Intent intent = new Intent(MainApp.getInstance(), PublishService.class);
		intent.setAction("org.aisen.weibo.sina.Cancel");
		PendingIntent contentIntent = PendingIntent.getService(MainApp.getInstance(), 0, intent, PendingIntent.FLAG_CANCEL_CURRENT);
	...
	public void notifyPublishCancelled(final PublishBean bean) {
		...
		// 延迟一秒后取消通知
		MainApp.getInstance().getHandler().postDelayed(new Runnable() {
