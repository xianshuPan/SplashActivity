...
public class FriendCircleAdapterNew extends IgBaseAdapter implements OnClickListener {
	...
	public FriendCircleAdapterNew(Activity activity) {
	...
	public FriendCircleAdapterNew(Activity activity, ArrayList<FriendHotItem> list, ListView listView, RefreshView refreshView, boolean isShowDelete) {

		mContext = activity;

		density = mContext.getResources().getDisplayMetrics().scaledDensity;
		this.list = list;
		mList = listView;
	...
	public View getView(int arg0, View arg1, ViewGroup arg2) {
		...
		if (arg1 ==  null) {
			...
			holder.praise = (ImageView) arg1.findViewById(R.id.praise_count_image);
			holder.praiseCountTxt = (TextView) arg1.findViewById(R.id.praise_count_Text);
			holder.praiseCountLinear = (LinearLayout) arg1.findViewById(R.id.praise_count_lineart);
			...
			holder.comments = (TextView)arg1.findViewById(R.id.comment_image);
		...
		/*是否有人点赞,点赞人的名字都要能够点击，*/
		if (list.get(arg0).praiseCount > 0) {

			holder.praiseCountTxt.setText(String.valueOf(list.get(arg0).praiseCount));
		}
		else {

			holder.praiseCountTxt.setText("");
		}
		holder.praisersLinear.removeAllViews();
		if (list.get(arg0).praises != null && list.get(arg0).praises.size() > 0) {

			LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams((int)(30*density), (int)(30*density));
			lp.setMargins(0, 0, (int) (5 * density), 0);

			holder.praisersLinear.setHorizontalSpacing((int)(5*density));

			int count = list.get(arg0).praises.size() > 15? 15:list.get(arg0).praises.size();

			for (int i = 0; i < count ; i++) {

				final String praiseSn = list.get(arg0).praises.get(i).get("sn");

				CircleImageView praiserName = new CircleImageView(mContext);

				praiserName.setLayoutParams(lp);
				praiserName.setPadding(5, 5, 5, 5);
				...
				//DownLoadImageTool.getInstance(mContext).displayImage(Utils.getAvatarURLString(praiseSn),praiserName,null);
				//Utils.loadAvatar(this,mData.student2_sn,mStudent2AvatarImage);
			}
			
			if (list.get(arg0).praises.size() > 15) {

				TextView praiserEndName = new TextView(mContext);
				praiserEndName.setText(R.string.str_friend_praised_end);
				praiserEndName.setTextColor(mContext.getResources().getColor(R.color.gray));
				praiserEndName.setLayoutParams(lp);
				praiserEndName.setGravity(Gravity.BOTTOM);
				holder.praisersLinear.addView(praiserEndName);
			}


			holder.praisersLinear.setVisibility(View.VISIBLE);
			
		/*没有人点赞*/
		}
		else {

			holder.praisersLinear.setVisibility(View.GONE);
		}
		
		if (list.get(arg0).praise == 0) {
			
			holder.praise.setImageResource(R.drawable.good);
			holder.praiseCountTxt.setTextColor(mContext.getResources().getColor(R.color.color_hint_txt));
			
		} else if (list.get(arg0).praise == 1) {
			
			holder.praise.setImageResource(R.drawable.good_clicked);
			holder.praiseCountTxt.setTextColor(mContext.getResources().getColor(R.color.color_yellow));
		}
		
		
		//final String tipsid = list.get(arg0).tipid;
		holder.praiseCountLinear.setOnClickListener(new OnClickListener() {
		...
		if (list.get(arg0).commentsCount > 0) {

			holder.comments.setText(String.valueOf(list.get(arg0).commentsCount));
		}
		else {
...
//		});
	...
	private static class ViewHolder {
		...
		public TextView praiseCountTxt ;
		public LinearLayout praiseCountLinear ;
			...
			protected void onPostExecute(Integer result) {
				...
				if(BaseRequest.REQ_RET_OK == result) {
					...
					if (praise == 0) {
						...
						list.get(mCurrentPositionInt).praises.add(0, reviewItemHash);

						list.get(mCurrentPositionInt).praiseCount = list.get(mCurrentPositionInt).praiseCount+1;
