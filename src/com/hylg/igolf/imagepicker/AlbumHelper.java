...
public class AlbumHelper {
	...
	void buildImagesBucketList() {
		...
		if (cur.moveToFirst()) {
			...
			allImageBucket.bucketName = Config.ALL_IMAGE; 
			
			String asdf = 	Environment.getExternalStorageDirectory().getAbsolutePath();
			String cameraPath = asdf+"/IGolf/camera.png";
			
			//String cameraPath = "";
			ImageItem imageCamera = new ImageItem();
			imageCamera.imageId = "19870313";
			imageCamera.imagePath = cameraPath;
			imageCamera.thumbnailPath = cameraPath;
			
			allImageBucket.imageList.add(imageCamera);
			//allImageBucket.count++;
			bucketList.put("198811041812", allImageBucket);
			...
			do {
				...
				if (cameraPath.equalsIgnoreCase(path)) {
	...
	String getOriginalImagePath(String image_id) {
		...
		Cursor cursor = cr.query(Media.EXTERNAL_CONTENT_URI, projection,
				Media._ID + "=" + image_id, null, Media.DATA);
		if (cursor != null) {
			cursor.moveToFirst();
