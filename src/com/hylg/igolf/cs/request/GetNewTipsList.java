...
public class GetNewTipsList extends BaseRequest {
	private String param;
	
	public ArrayList<FriendHotItem> hotList = null;

	public GetNewTipsList(Context context, String tipsIds, String sn, int pageNum, int pageSize) {
		super(context);
		hotList = new ArrayList<FriendHotItem>();
		StringBuilder s = new StringBuilder();
		s.append("tipsIds"); s.append(KV_CONN); s.append(tipsIds);
		s.append(PARAM_CONN);
	...
	public ArrayList<FriendHotItem> getMyFollowerList() {
		return hotList;
	}
	
	@Override
	public String getRequestUrl() {
		return getReqParam2("getTipsDetail", param);
	}
...
//			}
			...
			for(int i=0, len=ja.length(); i<len; i++) {
				...
				for (int j =0 ; j < comments.length() ; j++) {
					...
					reviewItemHash.put("tipid", reviewItem.optString("tipid"));
					reviewItemHash.put("sn", reviewItem.optString("sn"));
					reviewItemHash.put("avatar", reviewItem.optString("avatar"));
